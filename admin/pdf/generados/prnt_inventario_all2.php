...
<span class="span3"><?php echo "<b>INVENTARIO: Bcs_ext1</b><br>Fecha:&nbsp;".$fechahoy."</span>";?>
</td>
</tr>
...
<td style="width: 40%">Art&iacute;culo</td>
<td class="center" style="width: 20%">Existencia</td>
<td class="center" style="width: 20%">Existencia R</td>
</tr>

<?php
$consulta=("SELECT id_rango,nombre FROM rango WHERE habilitado=0 ORDER BY nombre");
$s2=mysqli_query($_SESSION['conexion_database'],$consulta) or die(mysqli_error($_SESSION['conexion_database']));
if(mysqli_num_rows($s2)<=0){
}else{   
while($row2=mysqli_fetch_array($s2)){
$id_rango=$row2['id_rango'];
$nombre_rango=$row2['nombre'];


$consulta3=("SELECT id_pieza, codigo_pieza, nombre FROM pieza WHERE id_rango='$id_rango' AND habilitado=0");
...
$referencia=$row3['codigo_pieza'];
$id_pieza=$row3['id_pieza'];
$nombre_pieza=$row3['nombre'];


$consulta4=("SELECT DISTINCT(id_pieza), cantidad FROM inventario WHERE id_pieza='$id_pieza' AND habilitado=0");
$s4=mysqli_query($_SESSION['conexion_database'],$consulta4) or die(mysqli_error($_SESSION['conexion_database']));
if(mysqli_num_rows($s4)<=0){
}else{   
while($row4=mysqli_fetch_array($s4)){
$id_pieza1=$row4['id_pieza'];
?>
<tr class="list_row">
<td style="text-align: left;" border="1px"><?php echo $referencia; ?></td>
<td style="text-align: left;" border="1px"><?php echo $nombre_pieza; ?></td>
<td style="text-align: center;" border="1px"><?php echo cantidad($id_pieza1); ?></td>
<td style="text-align: center;" border="1px"></td>
</tr>
<?php
}//while
}//else
